...
from dags.repo.ENCAR.utils.telegram_bot import send_data_to_group
...
def encar_parser_function(search_list_df):
    ...
    for index, search in search_list_df.iterrows():
        ...
        if not parse_result_df:
            ...
            # Пишем в телегу
            loop = asyncio.get_event_loop()
            loop.run_until_complete(send_data_to_group([search_id, ], 'ID, для которого парсинг выполнен с ошибкой'))
        else:
            ...
            not_existed_card_car_id = []
            to_update_df_car_id = []
            card_data_not_existed_df = pd.DataFrame()
            card_data_to_update_df = pd.DataFrame()
            card_data_bd_df = load_data_from_bd(logger, config, 'select_parsing_result_card_id.sql', base_dir, 'encar',
                                                'parsing_result_card')
            try:
